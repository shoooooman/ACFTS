...
\TODO{Mention the advantages of asynchronous systems.}
...
if the transaction itself in a request has no errors, they approve the transaction.
...
that is, $N > 3f$.
The smallest $N$ that saticefies this condition is $N=3f+1$.
From this result, we can see that $c_0$ is incapable of making the two conflicting transactions
approved under the condition that it has to get more than two-thirds of servers (i.e. $\geq2f+1$
for each transaction.
Actually, only one or neither transaction is approved.
